...
let printEndOfDayReport = (tillData, summaryData, tenderCounts, tenders, closingNotes, locationName) => {
...
const getPrintableAssetUrl = (value) => {
...
const printEndOfDayReportEnhanced = (tillData, summaryData, tenderCounts, tenders, closingNotes, locationName) => {
  const formatNaira = (amount) =>
  ...
  const openedAt = tillData?.openedAt
    ? new Date(tillData.openedAt).toLocaleTimeString('en-NG', {
        hour: '2-digit',
        minute: '2-digit',
        hour12: false,
      })
    : 'N/A';
  const closedAt = timeStr;
  const sessionId = tillData?._id ? String(tillData._id).slice(-8).toUpperCase() : 'N/A';
  const logoAbsolute = getPrintableAssetUrl(getStoreLogo());
  const totalPhysical = (tenders || []).reduce((sum, tender) => sum + (parseFloat(tenderCounts?.[tender.id]) || 0), 0);
  const totalExpected = Number(summaryData?.openingBalance || 0) + Number(summaryData?.totalSales || 0);
  const totalVariance = totalPhysical - totalExpected;
  const pendingSyncCount = Number(summaryData?.pendingLocalTransactions || 0);
  const varianceTone = totalVariance === 0 ? '#111827' : totalVariance > 0 ? '#065f46' : '#991b1b';
  const varianceText = totalVariance === 0 ? 'Balanced' : totalVariance > 0 ? 'Over' : 'Short';

  const tenderRows = (tenders || [])
    .map((tender) => {
      const expected = Number(summaryData?.tenderBreakdown?.[tender.name] || 0);
      const physical = Number(parseFloat(tenderCounts?.[tender.id]) || 0);
      const variance = physical - expected;
      const varianceLabel = variance === 0 ? 'BALANCED' : variance > 0 ? 'OVER' : 'SHORT';
      const varianceColor = variance === 0 ? '#111827' : variance > 0 ? '#065f46' : '#991b1b';

      return `
        <div class="tender-row">
          <div class="tender-main">
            <div class="tender-name">${escapeReportHtml(tender.name)}</div>
            <div class="tender-meta">Expected ${escapeReportHtml(formatNaira(expected))}</div>
          </div>
          <div class="tender-values">
            <div class="amount actual">${escapeReportHtml(formatNaira(physical))}</div>
            <div class="variance" style="color:${varianceColor};">${escapeReportHtml(formatNaira(variance))} ${varianceLabel}</div>
          </div>
        </div>
      `;
    })
...
<body>
  <div class="report-page">
    <div class="report">
      <div class="header">
        ${logoAbsolute ? `<img src="${escapeReportHtml(logoAbsolute)}" class="logo" alt="Logo" onerror="this.style.display='none'">` : ''}
        <div class="title">End of Day Report</div>
        <div class="subtitle">${escapeReportHtml(locationName || 'Store Location')}</div>
        <div class="subtitle">${escapeReportHtml(`${dateStr} ${timeStr}`)}</div>
      </div>

      <div class="session-card">
        <div class="meta-row">
          <span class="meta-label">Session</span>
          <span class="meta-value">${escapeReportHtml(sessionId)}</span>
        </div>
        <div class="meta-row">
          <span class="meta-label">Opened</span>
          <span class="meta-value">${escapeReportHtml(openedAt)}</span>
        </div>
        <div class="meta-row">
          <span class="meta-label">Closed</span>
          <span class="meta-value">${escapeReportHtml(closedAt)}</span>
        </div>
        <div class="meta-row">
          <span class="meta-label">Staff</span>
          <span class="meta-value">${escapeReportHtml(tillData?.staffName || 'N/A')}</span>
        </div>
        <div class="meta-row">
          <span class="meta-label">Transactions</span>
          <span class="meta-value">${escapeReportHtml(String(tillData?.transactionCount || 0))}</span>
        </div>
        ${pendingSyncCount > 0 ? `
          <div class="meta-row">
            <span class="meta-label">Pending Sync</span>
            <span class="meta-value">${escapeReportHtml(String(pendingSyncCount))}</span>
          </div>
        ` : ''}
      </div>
      ...
      <div class="section">
        ...
        <div class="summary-card">
          <div class="summary-row">
            ...
            <span class="amount">${escapeReportHtml(formatNaira(summaryData?.openingBalance))}</span>
          </div>
          <div class="summary-row">
            <span>Total Sales</span>
            <span class="amount">${escapeReportHtml(formatNaira(summaryData?.totalSales))}</span>
          </div>
          <div class="summary-total">
            <span>Expected Closing</span>
            <span class="amount">${escapeReportHtml(formatNaira(totalExpected))}</span>
          </div>
        </div>
      </div>
      ...
      <div class="section">
        <div class="section-heading">
          <span class="section-title">Tender Reconciliation</span>
          <span class="section-kicker">${escapeReportHtml(`${(tenders || []).length} tender${(tenders || []).length === 1 ? '' : 's'}`)}</span>
        </div>
        <div class="tender-list">
          ${tenderRows || '<div class="tender-row"><div class="tender-main"><div class="tender-name">No tender data</div></div><div class="tender-values"><div class="actual amount">\u20A60.00</div></div></div>'}
        </div>
      </div>
      ...
      <div class="section">
        <div class="summary-card">
          <div class="summary-row">
            ...
            <span class="amount">${escapeReportHtml(formatNaira(totalPhysical))}</span>
          </div>
          <div class="summary-total">
            <span>Total Variance</span>
            <span class="amount" style="color:${varianceTone};">${escapeReportHtml(formatNaira(totalVariance))}</span>
          </div>
        </div>
        <div class="variance-card">
          <span>Reconciliation Status</span>
          <strong class="amount" style="color:${varianceTone};">${escapeReportHtml(varianceText)}</strong>
        </div>
        <div class="status-chip-wrap">
          <div class="status-chip">${escapeReportHtml(varianceText)}</div>
        </div>
      </div>
      ...
      ${closingNotes ? `
        <div class="section">
          <div class="section-heading">
            <span class="section-title">Closing Notes</span>
          </div>
          <div class="notes">${escapeReportHtml(closingNotes)}</div>
        </div>
      ` : ''}

      <div class="footer">
        <div>Printed ${escapeReportHtml(`${dateStr} ${timeStr}`)}</div>
        <div style="margin-top: 0.8mm;">Powered by BizSuits</div>
      </div>
    </div>
  </div>
</body>
  ...
  try {
    ...
    const doPrint = () => {
      ...
      setTimeout(() => {
        try { document.body.removeChild(iframe); } catch (error) { /* ignore */ }
      }, 5000);
    };
    ...
    console.error('Failed to print end-of-day report:', error);
  }
};

printEndOfDayReport = printEndOfDayReportEnhanced;
